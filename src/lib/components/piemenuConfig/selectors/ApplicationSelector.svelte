...
<div class="mt-3 space-y-1">
    ...
    <div class="flex items-stretch space-x-2">
        ...
        <select
                id="appNameSelect"
                class="custom-select block w-full pl-3 py-2 border-none focus:outline-none focus:ring-2 focus:ring-amber-400 text-sm rounded-lg shadow-sm bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 transition-colors custom-select {selectedAppName ? 'text-zinc-900 dark:text-zinc-100' : 'text-zinc-500 dark:text-zinc-500'}"
                value={selectedAppName}
                onchange={handleChange}
        >
            <option value="" disabled={selectedAppName !== ''}
                    class="bg-zinc-200 dark:bg-zinc-800 text-amber-500 dark:text-amber-500">Select an application...
            </option>
